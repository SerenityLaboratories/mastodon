...
class ConversationAccount < ApplicationRecord
  ...
  class << self
    ...
    def remove_status(recipient, status)
      conversation = find_by(account: recipient, conversation_id: status.conversation_id, participant_account_ids: participants_from_status(status))

      return if conversation.nil? || conversation.last_status_id != status.id

      while last_status_id = conversation.status_ids.pop do
