...
describe StatusLengthValidator do
  describe '#validate' do
    ...
    it 'adds an error when content warning is over 1000 characters' do
      status = instance_double(Status, spoiler_text: 'a' * 1040, text: '', errors: activemodel_errors, local?: true, reblog?: false)
      subject.validate(status)
      expect(status.errors).to have_received(:add)
    end

    it 'adds an error when text is over 1000 characters' do
      status = instance_double(Status, spoiler_text: '', text: 'a' * 1040, errors: activemodel_errors, local?: true, reblog?: false)
      subject.validate(status)
      expect(status.errors).to have_received(:add)
    end
